...
import type { Membership, TimeEntry, HoursSummary } from "./types.js";
 ...
 */
function entryHours(entry: TimeEntry, breakMinutes = 0): number {
 ...
 * later if needed.
 */
export function sollHoursForRange(membership: Membership, dateStart: Date, dateEnd: Date): number {
...
export async function computeHoursSummary(
    ...
    const istHours = (entries as any[]).reduce((sum, entry) => {
        const breakMinutes = entry.shifts?.break_minutes ?? 0;
        return sum + entryHours(entry as TimeEntry, breakMinutes);
    }, 0);
