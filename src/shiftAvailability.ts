...
export interface MinuteRange {
    start: number;   // minutes since epoch (local)
...
function timeToMinutesOfDay(timeStr: string): number {
 ...
 */
export function getShiftRange(shiftDate: string, startTime: string, endTime: string): MinuteRange {
 ...
 */
export function rangeToShiftFields(
    range: MinuteRange,
    anchorDate: string
...
export async function findShiftConflicts(
...
): Promise<{ conflicts: Shift[]; error: unknown | null }> {
    ...
    const conflicts = (data as Shift[]).filter(existing => {
