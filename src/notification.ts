...
let checkerHandle: number | undefined;
...
export function startShiftReminders(): void {

    if(!isNotificationSupported()) return;
    if(Notification.permission !== "granted") return;
    if(checkerHandle !== undefined) return;
...
export function stopShiftReminders(): void {
...
async function checkCurrentShift(): Promise<void> {
    ...
    (data as Shift[]).forEach(shift => {
        ...
        if(minutesToStart > 0 && minutesToStart <= REMINDER_WINDOW_MINUTES){
            ...
            if(!alreadyNotified(key)){
                fireNotification(
                    "Upcoming shift",
                    `Your shift starts in ${Math.round(minutesToStart)} min — don't forget to check in.`
                );
                markNotified(key);
            }
        }
        ...
        if(minutesToEnd > 0 && minutesToEnd <= REMINDER_WINDOW_MINUTES){
            ...
            if(!alreadyNotified(key)){
                fireNotification(
                    "Shift ending soon",
                    `Your shift ends in ${Math.round(minutesToEnd)} min — don't forget to check out.`
                );
                markNotified(key);
            }
        }
