...
function actionClass(action: string): string {
    if(action.startsWith("attendance.")) return "act-attendance";
    if(action.startsWith("settings."))   return "act-settings";
    if(action.startsWith("crew."))       return "act-crew";
    if(action.startsWith("shift."))      return "act-shift";
    if(action.startsWith("auth."))       return "act-auth";
    if(action.startsWith("export."))     return "act-export";
    return "act-system";
}
...
function baseQuery(forCount = false) {
    ...
    let q = supabaseClient
        .from("activity_log")
        .select(
            "*, memberships:actor_membership_id(full_name)",
...
async function refresh(): Promise<void> {
    ...
    const countRes = await baseQuery(true) as any;
    totalCount = countRes.count ?? 0;

    clampCurrentPage();

    let q = baseQuery(false) as any;
    ...
    handle.addEventListener("mousedown", e => {
        ...
        const onMove = (ev: MouseEvent) => {
            ...
            const col = document.getElementById("historyColgroup")
                ?.children[index] as HTMLElement | undefined;
...
function renderBody(): void {
    ...
    rows.forEach(r => {
        ...
        main.innerHTML = `
            ...
            <td><span class="action-badge ${actionClass(r.action)}">${
                escapeHtml(ACTION_LABELS[r.action] ?? r.action)
            }</span></td>
...
function buildDetailRow(r: any): HTMLTableRowElement {
...
async function exportCsv(): Promise<void> {
    ...
    await logActivity(
        "export.performed",
        `Exported ${all.length} history entries (CSV)`,
        { rows: all.length, filter: filterAction, search: activeSearch }
    );
